...
Marine sediment TOC profiles almost always decrease monotonically with
depth as organic matter degrades. Lake sediments can behave very
differently. A lake whose productivity varied through the Holocene —
perhaps peaking during a warm, wet mid-Holocene optimum — can show a TOC
profile that rises with depth before eventually declining. Here we
construct a profile representative of such a lake: ~5% TOC at the
surface, rising to ~15% in the mid-Holocene, and returning to ~5% by the
early Holocene.
...
![](bacterial-correction_files/figure-html/lake_toc_profile-1.png)

Notice that TOC at 250 cm (mid-Holocene) is three times the surface
value. Because the model normalises bacterial resetting to surface TOC,
the `TOC_z / TOC_0` ratio reaches 3 at that depth, making bacterial
resetting *three times stronger* there than at the surface.
...
![](bacterial-correction_files/figure-html/lake_dl_profiles-1.png)

The D/L profiles develop a visible inflection or dip near 250 cm at
higher `f_bac` values. The curve is no longer monotonically increasing
with depth. At 500 cm the profiles converge back toward the abiotic case
because TOC has returned to its surface value (ratio = 1) and the
early-Holocene sediment experienced lower resetting rates throughout its
burial history.

### The spurious temperature signal

This is the key problem for paleothermometry: an abiotic inversion
applied to a bacterially reworked D/L profile would interpret the
mid-Holocene D/L dip as a real temperature signal. We can estimate the
implied apparent temperature at each depth using the same grid-search
approach as above.
...
![](bacterial-correction_files/figure-html/lake_temp_bias-1.png)

The apparent temperature is not simply cooler everywhere: it has a
structured depth pattern that mirrors the TOC hump. Near the surface,
apparent temperature is slightly cool (resetting rate proportional to 5%
TOC). In the mid-Holocene, bacterial resetting is strongest (15% TOC,
ratio = 3), producing the largest apparent cooling, up to several
degrees C for `f_bac = 5e-3`. Below 250 cm, as TOC declines back to 5%,
the bias shrinks.

The practical implication: **a Holocene productivity event can
masquerade as a Holocene temperature event in the D/L record**. If a
lake has independent evidence of mid-Holocene high productivity (e.g.,
high biogenic silica, high organic matter flux), this correction should
be applied before attempting a temperature reconstruction.
...
- **Productive lake sediments with non-monotonic TOC**: as shown above,
  a mid-Holocene productivity peak can create a structured apparent
  temperature signal that resembles real Holocene climate variability.
  If independent proxies (biogenic silica, organic matter flux, pollen)
  indicate past productivity changes, a measured downcore TOC profile
  should be used rather than a simple decay model.
